var DataTableIndex = $('#DataTable_upload').DataTable({
    'language': {
        'url': datatable_lang,
    },
    // 'stateSave': true,
    ...
    'drawCallback': function( settings ) {
        ...
        $('.level_change').on('click', function(i) {
            var data_pk = [];
            data_pk = [$(this).attr('data-pk')];
            $('.noty_layout').remove(); // close jsnotif
            getLangJSON().done(function(lang){
                cfSwalCustom(lang.modal['change_level_title'],lang.modal['change_level_content'] , lang.modal['yes'], null ,data_pk,api_table,admin_url+a_mod+'/level_change');
            });
        });
