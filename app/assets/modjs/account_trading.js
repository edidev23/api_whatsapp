...
$('#form_connecting').on('submit', function(event) {
    event.preventDefault();
    $('.connecting').find('i#submit_icon').removeClass().addClass('icon-spinner2 spinner mr-2');
    // $('.noty_layout').remove();
    tinyMCE.triggerSave();
    var form = $('#form_connecting');
    $.ajax({
        url: admin_url + a_mod + '/connecting_vps',
        type: 'POST',
        data: form.serialize(),
        dataType: 'json',
        cache: false,
        success: function(data) {
            cfNotif(data['alert']);
            $('.connecting').find('i#submit_icon').removeClass().addClass('fa fa-save mr-2');
            // $('html, body').animate({scrollTop:0});
        }
    })
    return false;
